...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between items-center h-24 mx-auto px-4 text-white bg-black rounded-xl w-[98%] m-4'>
      ...
      <button className='bg-[#2075f0] rounded p-2'>CP Streak</button>
      ...
      <div className={(nav) ? 'fixed left-0 top-0 w-[60%] h-full border-r bg-[#110011] border-r-gray-900 ease-in-out duration-500 z-50 ' : 'fixed left-[-100%] z-50 '}>
        <ul className='pt-12 uppercase p-4'>
          <li className='p-4 border-b border-gray-600' onClick={() => handleNav()}><Link href="/">Home</Link></li>
          <li className='p-4 border-b border-gray-600' onClick={() => handleNav()}><Link href="/About">About us</Link></li>
          <li className='p-4 border-b border-gray-600' onClick={() => handleNav()}><Link href="/Contact">Contact us</Link></li>
